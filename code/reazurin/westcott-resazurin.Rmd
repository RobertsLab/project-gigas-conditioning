...
for (file in file_list) {
  ...
  data_long <- data %>%
    ...
    dplyr::select(FileName, plate, Well_ID, Value, date, timepoint)  # Select relevant columns
...
```{r}
auc<-full_data%>%
  dplyr::select(date, plate, well, individual, temperature, timepoint, group, experiment, treatment, value)%>%
...
plot4<-auc%>%
  ...
  ggplot(aes(x=temperature, y=mean, linetype=treatment))+
...
ggsave(plot4, filename="figures/resazurin/westcott_mean_AUC_updated.png", width=10, height=5, units="in")

plot4a<-auc%>%
  
  ggplot(aes(x=temperature, y=AUC, colour=treatment))+
  facet_wrap(~experiment, scales="free_x")+
...
ggsave(plot4a, filename="figures/resazurin/westcott_dots_AUC.png", width=10, height=5, units="in")
```

```{r}
plot5<-auc%>%
  group_by(group, experiment, treatment, temperature)%>%
  dplyr::summarise(mean=mean(AUC, na.rm=TRUE), se=sd(AUC, na.rm=TRUE)/sqrt(length(AUC)))%>%
  ...
  scale_colour_manual(values=c("orange", "darkred"), name="")+
  theme_classic()+
