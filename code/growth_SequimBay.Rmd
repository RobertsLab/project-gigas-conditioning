...
```{r}
plot1<-data%>%
  ggplot(aes(x=bag, y=length.mm, fill = treatment)) +
  facet_wrap(~date)+
...
```{r}
plot2<-data%>%
  ggplot(aes(x=bag, y=width.mm, fill = treatment)) +
...
plot5<-data%>%
  ...
  geom_point(pch = 21, position=position_jitterdodge(dodge.width=0.4)) +
  xlab("Treatment") +
  ylab(expression(paste("Growth (mm" ^3, ")")))+
  theme_classic() +
    scale_colour_manual(values=c("darkgray", "orange"))+
  scale_fill_manual(values=c("darkgray", "orange"))+
  theme(axis.text.x=element_text(angle=45, vjust=1, hjust=1), 
        legend.title=element_blank()); plot5

ggsave(plot5, filename="figures/growth/sequim/treatment_sizes.png", width=5, height=4, units="in")
```
...
Treated oysters are similar to untreated siblings. 

## Run linear mixed models models 

```{r}
model<-lmer(sqrt(Predicted_Volume_Poly) ~ treatment + (1|bag:treatment) + (1|date), data=data)
...
```{r}
data%>%
  dplyr::select(date, bag, oyster, treatment, Predicted_Volume_Poly)%>%
  mutate(treatment=if_else(treatment=="Treated", "Temperature", "Control"))%>%
  write_csv("output/sequim_growth.csv")
```
