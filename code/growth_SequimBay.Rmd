...
training_data<-read_csv(file="data/outplanting/GoosePoint/growth_GoosePoint.csv")%>%
  mutate(date=as.factor(date))%>%
  filter(date %in% c("20240909", "20250825", "20260522"))
...
```{r}
model<-lmer(sqrt(Predicted_Volume_Poly) ~ treatment*date + (1|bag:treatment), data=data)
...
data%>%
  ...
  mutate(experiment=c("Temperature"))%>%
