...
plot2<-survival%>%
  ...
  dplyr::mutate(total_dead=rowSums(across(7:12)))%>%
...
survival%>%
  ...
  pivot_wider(names_from="date", values_from="dead")%>%
  dplyr::mutate(total_dead=rowSums(across(7:12)))%>%
  select(corrected_tag, treatment, group, experiment, total_dead)%>%
  mutate(starting_oysters=c(100))%>%
  mutate(percent_survival=(starting_oysters-total_dead)/starting_oysters)%>%
  mutate(treatment=if_else(treatment=="control", "Control", 
                           if_else(experiment=="weekly temperature" & treatment=="treated", "Treated", 
                                   if_else(experiment=="weekly fresh water" & treatment=="treated", "Treated", NA))))%>%
  write_csv("output/goosepoint_survival.csv")
```
...
```{r}
test_data1<-survival%>%
  filter(experiment=="weekly temperature")%>%
  select(field_cattle_tag, tag_20250825, corrected_tag, date, treatment, group, experiment, dead)%>%
  pivot_wider(names_from="date", values_from="dead")%>%
  dplyr::mutate(total_dead=rowSums(across(7:12)))

kruskal.test(total_dead ~ treatment, data=test_data1) #no effect 

test_data2<-survival%>%
  filter(experiment=="weekly fresh water")%>%
  select(field_cattle_tag, tag_20250825, corrected_tag, date, treatment, group, experiment, dead)%>%
  pivot_wider(names_from="date", values_from="dead")%>%
  dplyr::mutate(total_dead=rowSums(across(7:12)))

kruskal.test(total_dead ~ treatment, data=test_data2) #no effect 
...
kruskal.test(total_dead ~ treatment, data=test_data3) #no effect 
```
...
```{r}
plot1<-data%>%
  ggplot(aes(x=corrected_tag, y=length.mm, fill = corrected_tag)) +
  facet_wrap(~date)+
...
```{r}
plot2<-data%>%
  ggplot(aes(x=corrected_tag, y=width.mm, fill = corrected_tag)) +
  facet_wrap(~date)+
  geom_point(pch = 21, position=position_jitterdodge(dodge.width=0.4)) +
  xlab("Bag") + 
  theme_classic(); plot2
...
```{r}
plot3<-data%>%
  ggplot(aes(x=corrected_tag, y=depth.mm, fill = corrected_tag)) +
  facet_wrap(~date)+
  geom_point(pch = 21, position=position_jitterdodge(dodge.width=0.4)) +
  xlab("Bag") + 
  theme_classic(); plot3
...
Obtain training data with known length width and depth from 20240909, 20251008, and 20250522 datasets. 
```{r}
training_data<-data%>%
  filter(date %in% c("20240909", "20251008", "20260522"))%>%
  filter(!is.na(volume))
...
summary(gam_model)
```
Poly model adjusted R sq = 0.961
GAM model adjusted R sq = 0.96 
...
```{r}
data<-data%>%
  filter(!is.na(Predicted_Volume_Poly))%>%
  filter(Predicted_Volume_Poly>0)
```
...
```{r}
# Plot polynomial fit for Length vs. Volume
ggplot(data%>%filter(date %in% c("20240909","20251008", "20260522")), aes(x = volume, y = Predicted_Volume_Poly)) +
  geom_point() +
...
min(data$Predicted_Volume_Poly, na.rm=TRUE)
```
...
Conduct outlier removal and analyze again.  
...
Don't remove outliers as it is identifying the larger timepoint as largely outliers. 
...
summary(poly_model)
```
R sq still 0.961 without removing outliers. 
...
```{r}
# Plot polynomial fit for Length vs. Volume
ggplot(data%>%filter(date %in% c("20240909", "20250825", "20251008", "20260522")), aes(x = volume, y = Predicted_Volume_Poly)) +
  geom_point() +
  stat_smooth(method = "loess", color = "blue", se = TRUE) +
  ggtitle("Polynomial Regression (2nd Degree) Fit for Known vs Predicted Volume")
```
...
Residuals are a little better, need to come back and re assess. 
...
Control bigger in august, no other differences. 
...
Control smaller in August than treated, treated smaller than control in October.  
...
fw  <- ggpred[ggpred$facet == "weekly fresh water", , drop = FALSE]
temp <- ggpred[ggpred$facet == "weekly temperature", , drop = FALSE]

#temperature
plot1<-ggplot(temp, aes(x = x, y = predicted, color = group, fill=group)) +
  ...
  labs(title = "",
       x = "Date",
       y = expression(paste("Growth (mm" ^3, ")")),
       color = "Treatment", 
       fill = "Treatment");plot1
```

Run model for just temperature. 
```{r}
# Fit a mixed-effects model to estimate growth rate
data2<-data%>%
  filter(experiment=="weekly temperature")

growth_model2 <- lmer(Predicted_Volume_Poly ~ date * treatment + (1|treatment:corrected_tag), data = data2)
...
```{r}
ggsave(filename="figures/growth/goose_point/growth_temp.png", plot1, width=6, height=6)
```

```{r}
#temperature
plot1<-ggplot(fw, aes(x = x, y = predicted, color = group, fill=group)) +
  geom_line(aes(group=group)) +
  geom_ribbon(aes(ymin = conf.low, ymax = conf.high, group=group), alpha = 0.2) +
  theme_classic() +
...
Run model for just fresh water 
```{r}
# Fit a mixed-effects model to estimate growth rate
data3<-data%>%
  filter(experiment=="weekly fresh water")

growth_model3 <- lmer(Predicted_Volume_Poly ~ date * treatment + (1|treatment:corrected_tag), data = data3)

# Summarize the model
summary(growth_model3)
anova(growth_model3)
```

```{r}
ggsave(filename="figures/growth/goose_point/growth_fw.png", plot1, width=6, height=6)
```
...
```{r}
data%>%
  dplyr::select(date, field_cattle_tag, oyster, treatment, Predicted_Volume_Poly, treatment, experiment)%>%
  mutate(treatment=if_else(treatment=="control", "Control", 
                           if_else(experiment=="weekly temperature" & treatment=="treated", "Treated", 
                                   if_else(experiment=="weekly fresh water" & treatment=="treated", "Treated", NA))))%>%
  write_csv("output/goosepoint_growth.csv")
```
