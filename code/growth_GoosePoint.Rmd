...
plot<-survival%>%
  ...
  geom_point()+
  geom_line(aes(group=field_cattle_tag))+
  theme_classic(); plot
```
...
```{r}
plot2<-survival%>%
  select(field_cattle_tag, tag_20250825, date, treatment, group, experiment, dead)%>%
  pivot_wider(names_from="date", values_from="dead")%>%
  #group_by(field_cattle_tag, tag_20250825, treatment, group, experiment)%>%
  dplyr::mutate(total_dead=rowSums(across(6:10)))%>%
