...
Read in Hobo Tidbit files  
```{r}
westcott.files <- list.files(path="data/environmental/loggers/westcott/westcottloggers_20251202/", pattern="*.xlsx", full.names=TRUE)

#will need to change to HST time 
westcott <- setNames(westcott.files, westcott.files) %>%
    map_dfr(read_xlsx, .id = "logger") %>%
    rename(DateTime=`Date-Time (PDT/PST)`, temp.C=`Temperature , °C`)%>%
    select(!`#`)%>%
    select(logger, DateTime, temp.C)%>%
    mutate(logger=sub(".*/", "", logger))%>% #remove the path name
    mutate(logger=sub("_.*", "", logger))%>% #keep only the serial number 
...
#export data 
westcott%>%
  write_csv("output/environmental/westcott-temperature-data.csv")
```
...
Make percentile rolling windows for graph (centered)
```{r}
#90th
westcott <- westcott %>%
  arrange(DateTime) %>%
  mutate(
    p90_30d_centered = slide_index_dbl(
      .x = temp.C,
      .i = DateTime,
      .f = ~ quantile(.x, probs = 0.9, na.rm = TRUE),
...
westcott <- westcott %>%
  arrange(DateTime) %>%
  mutate(
    p50_30d_centered = slide_index_dbl(
      .x = temp.C,
      .i = DateTime,
      .f = ~ quantile(.x, probs = 0.5, na.rm = TRUE),
      .before = lubridate::days(15),
      .after  = lubridate::days(15),
      .complete = TRUE
    )
...
westcott <- westcott %>%
  arrange(DateTime) %>%
  mutate(
    p10_30d_centered = slide_index_dbl(
      .x = temp.C,
      .i = DateTime,
      .f = ~ quantile(.x, probs = 0.1, na.rm = TRUE),
      .before = lubridate::days(15),
      .after  = lubridate::days(15),
      .complete = TRUE
    )
...
westcott_plot<-westcott%>%
  filter(DateTime > "2024-07-17 00:00:00")%>%
  
  ggplot(aes(x=DateTime, y=temp.C))+
  geom_point(alpha=0.7, colour ="gray")+
  geom_line(
    aes(y = p90_30d_centered),
    color = "red",
    linewidth = 1.2,
    linetype = "dashed",
    na.rm = TRUE
...
Remove temperature data from read out days (6/24, 9/9, 12/11, 08/25)

```{r}
# Dates to filter out
dates_to_filter <- as.Date(c("2024-06-19", "2024-06-20", "2024-06-21", "2024-06-22", "2024-06-23", "2024-06-24", "2024-06-25", "2024-09-09", "2024-09-10", "2024-12-11", "2025-05-20", "2025-08-25", "2025-10-08", "2025-05-22"))
...
Read in Hobo file  
```{r}
sequim.files <- list.files(path="data/environmental/loggers/sequim", pattern="*.xlsx", full.names=TRUE)
...
Remove temperature data from prior to outplant. 
...
All loggers for summer
```{r}
sequim_plot<-sequim%>%
  filter(day > as.Date(c("2025-04-03")))%>%
