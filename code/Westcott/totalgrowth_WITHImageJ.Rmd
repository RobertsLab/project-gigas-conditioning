...
```{r}
# B survival
B_growth <- read_xlsx("data/sizes/B_2023POGS_small_seed_daily/B_sizes_12022025.xlsx")
...
# D survival
D_growth <- read_xlsx("data/sizes/D_2023POGS_small_seed_weekly/D_sizes_12022025.xlsx", guess_max = 2000)
...
Dec2_sizes <- read_xlsx("data/outplanting/Westcott/20251202_images/sizes_20251202.xlsx")
```
...
# Dec sizes 2025
Dec2_sizes <- Dec2_sizes %>%
  pivot_wider(names_from = metric,
              values_from = c(measure.mm))

names(Dec2_sizes)[names(Dec2_sizes) == "length"] <- "length.mm"
Dec2_sizes <- subset(Dec2_sizes, select=-c(oyster))
...
growth <- bind_rows(B_growth_sum, D_growth_sum, Dec_sizes, May_sizes, June_sizes, July_sizes, Aug_sizes, Dec2_sizes)
...
growth$conditioning <- as.factor(growth$conditioning)
...
grid.arrange(B_length_graph,D_length_graph)
```
...
#output growth data frame
growth%>%
  select(effort, conditioning, date, bag_tag_num, vol)%>%
  mutate(experiment=if_else(effort=="B", "Daily Temperature", 
                            if_else(effort=="D", "Weekly Temperature", NA)))%>%
  mutate(treatment=if_else(conditioning=="treated", "Treated", 
                           if_else(conditioning=="control", "Control", NA)))%>%
  select(experiment, treatment, date, bag_tag_num, vol)%>%
  write_csv("output/westcott_growth.csv")
...
```{r}
  ggplot(mean_vols, aes(x = date, y = mean_vol, color = effort)) +
  geom_point()+
  geom_line(aes(linetype=conditioning))+
  labs(x = "Date",
       y = "Mean volume ") +
...
hist(growth$length.mm)

model.l <-glmmTMB(length.mm ~  effort/conditioning*date + (1|bag_tag_num) ,data = growth, family = gaussian())
summary(model.l)
car::Anova(model.l, type = "III")
tst_simres <- simulateResiduals(fittedModel = model.l) #DHARMa
plot(tst_simres, quantreg = T)
testDispersion(model.l)
qqPlot(residuals(model.l))

plot(growth$date, growth$vol)

model.v <-glmmTMB(vol ~  effort/conditioning*date + (1|bag_tag_num) ,data = growth, dispformula = ~date , family = tweedie())
summary(model.v)
car::Anova(model.v)
tst_simres <- simulateResiduals(fittedModel = model.v) #DHARMa
plot(tst_simres, quantreg = T)
testDispersion(model.v)
...
model.a <-glmmTMB(mean_vol ~  effort/conditioning *days+ (1|bag_tag_num),data = means_vols, family = gaussian()) #glmmTMB
summary(model.a)
...
# volume
model.w<-glmmTMB(mean_vol ~  conditioning *days+ (1|bag_tag_num),data = vols_w, family =gaussian()) #glmmTMB
summary(model.w)
...
model.d<-glmmTMB(mean_vol ~  conditioning *days+ (1|bag_tag_num),data = vols_d, family = gaussian()) #glmmTMB
summary(model.d)
...
# length
l_w <- growth %>% filter(effort == "D")
l_d <- growth %>% filter(effort == "B")

model.wl<-glmmTMB(length.mm ~  conditioning *date+ (1|bag_tag_num),data = l_w, family =gaussian()) #glmmTMB
summary(model.wl)
car::Anova(model.wl,type = "II")
qqPlot(residuals(model.wl))

testDispersion(model.wl)
tst_simres <- simulateResiduals(fittedModel = model.wl) #DHARMa
plot(tst_simres, quantreg = T)

model.dl <-glmmTMB(length.mm  ~  conditioning*date+ (1|bag_tag_num),data = l_d, family = gaussian()) #glmmTMB
summary(model.dl)
car::Anova(model.dl,type = "II")
qqPlot(residuals(model.dl))

testDispersion(model.dl)
tst_simres <- simulateResiduals(fittedModel = model.dl) #DHARMa
plot(tst_simres)

```


# volume model no zeros
```{r}
vols_d_no0 <- vols_d %>%
  filter(days >0)
...
hist(vols_w_no0$perc_change_vol)

model.w.0 <-glmmTMB(perc_change_vol ~  conditioning *days+ (1|bag_tag_num),data = vols_w_no0, family =lognormal()) #glmmTMB
summary(model.w.0)
car::Anova(model.w.0,type = "III")
qqPlot(residuals(model.w.0))

testDispersion(model.w.0)
tst_simres <- simulateResiduals(fittedModel = model.w.0) #DHARMa
plot(tst_simres)


model.d.0 <-glmmTMB(perc_change_vol ~  conditioning *days+ (1|bag_tag_num),data = vols_d_no0, family =lognormal()) #glmmTMB
summary(model.d.0)
car::Anova(model.d.0,type = "III")
qqPlot(residuals(model.d.0))

testDispersion(model.d.0)
tst_simres <- simulateResiduals(fittedModel = model.d.0) #DHARMa
plot(tst_simres)
...
library(emmeans)
emm_interaction <- emmeans(model.a, ~ effort * conditioning | days)
emm_interaction
...
ggsave("figures/growth/westcott/height_stack.pdf", plot = grid, width = 7, height = 10, units = "in")
