...
```{r}
# B survival
B_survival <- read_excel("data/survival/Westcott/B_survival_12022025.xlsx")
B_survival$conditioning <- gsub("daily", "treated", B_survival$conditioning)

# D survival
D_survival <- read_excel("data/survival/Westcott/D_survival_12022025.xlsx")
...
```{r}
survival%>%
  select(effort, conditioning, date, bag_tag_num, proportion_remaining)%>%
  ...
  select(experiment, treatment, date, bag_tag_num, proportion_remaining)%>%
  mutate(proportion_remaining=if_else(proportion_remaining>1, 1.00, proportion_remaining))%>%
  write_csv("output/westcott_survival.csv")
...
ggsave("figures/temp_surv_stack.pdf", plot = grid, width = 7, height = 10, units = "in")
```
